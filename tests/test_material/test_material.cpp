...
Vector3d RayColor(const Tracey::Ray& r, const Tracey::Geometry& scene,
  ...
  if (scene.intersect(r, 0, infinity, rec)) {
    ...
    Vector3d target = rec.p + rec.normal + Tracey::RandomInUnitSphere();
...
int main() {
  ...
  scene.add(std::make_shared<Tracey::Sphere>(Vector3d(0.0, -100.5, -1), 100));
    ...
    for (int i = 0; i < image_width; ++i) {
      ...
      for (int s = 0; s < samples_per_pixel; ++s) {
        ...
        Vector3d lower_left_corner =
            origin - (horizontal / 2) - (vertical / 2) - Vector3d(0, 0, 1.0);
        Tracey::Ray r(
